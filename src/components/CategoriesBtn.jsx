...
useEffect(()=> {
 ...
 const handleCategories = async (selectOption) => {
    ...
    try{
        const res = await axios.get(`https://style-tehran-shirt-nodejs.onrender.com/api/products?category=${selectOption.value}`)
...
const handleDelete = async (id) => {
    ...
    return(
      ...
      <div className="p-4   bg-gray-300" >
        {products.length > 0 ? (
         <div className=" flex flex-row max-h-screen md:grid md:grid-cols-6 gap-x-2 py-4 md:py-0 md:gap-x-10 gap-y-4 overflow-x-auto md:overflow-x-hidden  ">
            {products.map((p) => (
              <div
                key={p.id}
